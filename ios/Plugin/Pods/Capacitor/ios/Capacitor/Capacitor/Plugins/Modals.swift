...
public class Modals: CAPPlugin {
    ...
    @objc public func confirm(_ call: CAPPluginCall) {
        ...
        let alert = UIAlertController(title: title, message: message, preferredStyle: UIAlertControllerStyle.alert)
        alert.addAction(UIAlertAction(title: okButtonTitle, style: UIAlertActionStyle.default, handler: { (_) -> Void in
            call.success([
                "value": true
            ])
        }))
        alert.addAction(UIAlertAction(title: cancelButtonTitle, style: UIAlertActionStyle.default, handler: { (_) -> Void in
            call.success([
                "value": false
            ])
        }))
    ...
    func buildActionSheet(_ call: CAPPluginCall, title: String, message: String, options: JSArray) -> UIAlertController {
        ...
        for (index, option) in options.enumerated() {
            ...
            let action = UIAlertAction(title: title, style: style == "DESTRUCTIVE" ? .destructive : .default, handler: { (_) -> Void in
                call.success([
                    "index": index
                ])
            })
